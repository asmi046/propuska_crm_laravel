...
<html lang="ru">
    ...
    <body>
        <p>Здравствуйте, {{ $pass['truck_num'] }} - вышел постоянный пропуск на {{ $pass['pass_zone'] }} с {{ date("d.m.Y", strtotime($pass['valid_from'])) }} по {{ date("d.m.Y", strtotime($pass['valid_to'])) }} включительно.  Серия и номер пропуска {{ $pass['series'] }} {{ $pass['pass_number'] }} ({{ $pass['type_pass'] }})</p>
        <br/>
        <br/>
