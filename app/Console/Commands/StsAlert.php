...
use App\Models\Sts;
use App\Mail\Alert\StsAlertMail;
...
class StsAlert extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'alerts:sts';
     ...
     * @var string
     */
    protected $description = 'Оповещение о смене СТС';
    ...
    public function handle()
    {
        ...
        $this->info("Найдено ".count($all_numbers)." номеров");
        ...
        foreach($all_numbers as $item) {
            ...
            try {
                ...
                if ($deycount == -1 || $deycount >= 10) {
                    ...
                    Mail::to($adt_tosend)
                        ->later(now()->addMinutes($delay), new StsAlertMail($item->truc_number, $item->sts_data));

                    Sts::where('truc_number', $item->truc_number)->update(['last_message' => now()]);
